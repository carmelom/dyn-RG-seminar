...
\begin{frame}{TDGL}
 ...
 \item[(ii)] \begin{flalign*}
  \begin{cases}
   \sigma_k(\omega) \mapsto s^{1-\eta/2 + z}\ \sigma_{sk}(s^z\omega) \\
   \omega' = s^{z} \omega\ ;\ k' = sk 
  \end{cases} \\ 
  \Downarrow \\ \displaystyle -i\omega' \sigma_{k'} = -\frac{s^z\ \Gamma_{k'/s}}{r_0 + c\ (k'/s)^2}  \ \sigma_{k'} + s^{\eta/2 -1}\zeta_{k'/s}(s^{-z}\omega')
&&
 \end{flalign*}
